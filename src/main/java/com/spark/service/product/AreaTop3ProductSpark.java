package com.spark.service.product;
...
//		sqlContext.setConf("spark.sql.shuffle.partitions", "1000"); // 提高shuffle并行度
...
//		sqlContext.udf().register("remove_random_prefix", new RemoveRandomPrefixUDF(), DataTypes.StringType);
		...
		// 因为数据量比较小，所以将计算出来的最终数据collect()到本地
	...
	private static JavaPairRDD<Long, Row> getCityId2ClickActionRDDByDate(
		...
		// 第一个限定：click_product_id不为空的访问行为，就代表点击行为
	...
	private static void generateTempAreaPrdocutClickCountTable(
		...
		/**
		 * 双重group by
		 * 子查询中的group by会先将数据打散之后聚合，外层再group by会将之前聚合的结果再进行聚合。
		 * 从而解决数据倾斜问题。
		 */	
//		String _sql = 
//				"SELECT "
...
//				+ "GROUP BY product_id_area ";  
	...
	private static void generateTempAreaFullProductClickCountTable(SQLContext sqlContext) {
		...
		// 最好得到表字段：area, product_id, click_count, city_infos, product_name, product_status
...
//				+ "JOIN tmp_product_info pi ON tapcc.product_id=pi.product_id ";
	...
	private static JavaRDD<Row> getAreaTop3ProductRDD(SQLContext sqlContext) {
		// 技术点：开窗函数 row_number(),本地模式下 开窗函数不能用
