 ...
 * 
 * 可以使用自己定义的一些数据格式，比如String或可以自己定义的model，自己定义的类（必须可序列化）
...
public class SessionAggrStatAccumulator implements AccumulatorParam<String> {
	 ...
	 * 各个范围区间的统计数量的拼接，统一采用key=value|key=value的连接串的格式
	 */
	@Override
	 ...
	 * 这两个方法为主要实现：
	 * v1是我们初始化的那个连接串
	 * v2是我们在遍历session的时候，判断出某个session对应的区间，然后会用Constants.TIME_PERIOD_1s_3s
	 * 所以我们要做的事情就是在v1中，找到v2对应的value，累加1，然后再更新回连接串里面去
	 * 
	 */
	@Override
